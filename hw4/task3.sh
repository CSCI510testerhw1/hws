...
#print the data to terminal before we move on to calculate the average down the pipe
...
cat titanic.csv | grep  '^[^,]*,[^,]*,2' | grep  -P 'S\r$' | sed 's/female/F/' | sed 's/male/M/' | tee /dev/tty | grep -o  -E  '[MF],[0-9]+' | gawk -F, '{sum+=$2;count++} END {print "Average Age:",sum/count}'